...
static void stream_queue_push(StreamQueue *q, const uint8_t *data, size_t size) {
    size_t written = 0;
...
static bool internal_connect(SecureCtx *ctx, const char *url, uint8_t **pushBuf, size_t *pushSize) {
    ...
    if (head_end > 0) {
        ...
        if (size > head_end) {
            ...
            *pushBuf = malloc(*pushSize);
            if (*pushBuf) memcpy(*pushBuf, full + head_end, *pushSize);
        } else {
            *pushBuf = NULL;
            *pushSize = 0;
        }
        ...
        if (internal_connect(q->net, q->url, &initData, &initSize)) {
            ...
            while (!s_quit && !q->quit) {
                ...
                if (ret == MBEDTLS_ERR_SSL_WANT_READ || ret == MBEDTLS_ERR_SSL_WANT_WRITE) {
                    svcSleepThread(10 * 1000 * 1000); // 10ms wait is fine in this thread
                    continue;
                }
