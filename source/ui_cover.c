...
// proxy url base
static const char *PROXY_BASE =
    "https://tripletail-socket.blueberry.coffee/api/convert-image?url=";
...
static void cover_worker(void *arg) {
    ...
    while (!cover_quit && !s_quit) {
        ...
        if (changed) {
            // mark as tried immediately to prevent infinite retry loops on
            // failure
...
void UI_Cover_Init(void) {
    ...
    cover_quit  = false;
    coverThread = threadCreate(cover_worker, NULL, COVER_STACK_SIZE,
                               THREAD_PRIO_COVER, -1, false);
...
void UI_Cover_Draw(float x, float y, float w, float h) {
    if (hasCover && coverSheet) {
        ...
        float text_w, text_h;
        C2D_TextGetDimensions(&text, 0.5F, 0.5F, // text scale
                              &text_w, &text_h);

        const float text_x = x + (w - text_w) * 0.5F;
        const float text_y = y + (h - text_h) * 0.5F;

        C2D_DrawText(&text, C2D_WithColor, text_x, text_y,
