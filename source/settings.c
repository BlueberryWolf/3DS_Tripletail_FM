...
void settings_register_string(const char *key, char *buffer,
                              size_t buffer_size) {
    if (s_registry_count >= MAX_SETTINGS)
        return;
    s_registry[s_registry_count].key =
        key; // assumes literal string or persistent memory
        ...
        *val      = '\0'; // separate key from value
        ...
        for (int i = 0; i < s_registry_count; i++) {
            if (strcmp(key, s_registry[i].key) == 0) {
                ...
                case TYPE_STRING:
                    strncpy((char *) s_registry[i].value_ptr, val,
                            s_registry[i].size - 1);
                    ((char *) s_registry[i].value_ptr)[s_registry[i].size - 1] =
                        '\0';
                    break;
                case TYPE_INT:
...
void settings_save(void) {
    ...
    for (int i = 0; i < s_registry_count; i++) {
        ...
        case TYPE_STRING:
            fprintf(fp, "%s=%s\n", s_registry[i].key,
                    (char *) s_registry[i].value_ptr);
            break;
        case TYPE_INT:
            fprintf(fp, "%s=%d\n", s_registry[i].key,
                    *(int *) s_registry[i].value_ptr);
            break;
        case TYPE_BOOL:
            fprintf(fp, "%s=%s\n", s_registry[i].key,
                    (*(bool *) s_registry[i].value_ptr) ? "true" : "false");
            break;
        }
    }
