...
void UI_Chat_Draw(float x, float y, float w, float h) {
  ...
  for (int i = chat_store.count - 1; i >= start; i--) {
    ...
    // user
    C2D_DrawText(&m->userText, C2D_WithColor | C2D_AtBaseline, currentX, currentY, 0.5f, scale, scale, m->user_color_parsed);
    ...
    C2D_DrawText(&s_colonText, C2D_WithColor | C2D_AtBaseline, currentX, currentY, 0.5f, scale, scale, C2D_Color32(200, 200, 200, 255));
    currentX += colonW;

    // messag
    C2D_DrawText(&m->msgText, C2D_WithColor | C2D_AtBaseline, currentX, currentY, 0.5f, scale, scale, C2D_Color32(230,230,230,255));
