...
static void parse_meta_json(const char *json, size_t len) {
                ...
                while (cur < ret && tokens[cur].start < end) {
                    if (tokens[cur].type == JSMN_STRING) {
                        if (jsoneq(json, &tokens[cur], "title") == 0) {
                            json_token_str(json, &tokens[cur + 1], title,
                                           sizeof(title));
                        } else if (jsoneq(json, &tokens[cur], "artist") == 0) {
                            json_token_str(json, &tokens[cur + 1], artist,
                                           sizeof(artist));
                        } else if (jsoneq(json, &tokens[cur], "art") == 0) {
                            json_token_str(json, &tokens[cur + 1], art,
                                           sizeof(art));
                        }
    ...
    if (title[0]) {
        ...
        strncpy(current_metadata.artist, artist,
                sizeof(current_metadata.artist));
...
void metadata_refresh(void) {
    ...
    const char *sub =
        "{\"subs\": {\"station:tripletail\": {\"recover\": true}}}";
    net_send_ws(&ctx, sub);
    ...
    while (1) {
        ...
        if (r == MBEDTLS_ERR_SSL_WANT_READ || r == MBEDTLS_ERR_SSL_WANT_WRITE) {
            svcSleepThread(100 * 1000 *
                           1000); // 100ms polling for metadata is plenty
            continue;
        }
        if (r <= 0)
            break; // disconnected
...
disconnected:
    ...
    svcSleepThread(1000 * 1000 *
                   1000); // wait 1s before allowing potential reconnect
}
